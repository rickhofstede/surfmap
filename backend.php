<?php
	...
	for ($i = 0; $i < sizeof($sessionData->geoCoderData); $i++) {
		...
		if ($sessionData->geoCoderData[$i]->srcCountry[0] === -1 && stripos($entry, "UNKNOWN") === false && !in_array($entry, $geocodingQueue)) {
			array_push($geocodingQueue, $entry);
		}
		
		$entry = $sessionData->geoLocationData[$i][1]['COUNTRY'];
		if ($sessionData->geoCoderData[$i]->dstCountry[0] === -1 && stripos($entry, "UNKNOWN") === false && !in_array($entry, $geocodingQueue)) {
			array_push($geocodingQueue, $entry);
		}
		
		$entry = $sessionData->geoLocationData[$i][0]['COUNTRY'].", ".$sessionData->geoLocationData[$i][0]['REGION'];
		if ($sessionData->geoCoderData[$i]->srcRegion[0] === -1 && stripos($entry, "UNKNOWN") === false && !in_array($entry, $geocodingQueue)) {
			array_push($geocodingQueue, $entry);
		}
		
		$entry = $sessionData->geoLocationData[$i][1]['COUNTRY'].", ".$sessionData->geoLocationData[$i][1]['REGION'];
		if ($sessionData->geoCoderData[$i]->dstRegion[0] === -1 && stripos($entry, "UNKNOWN") === false && !in_array($entry, $geocodingQueue)) {
			array_push($geocodingQueue, $entry);
		}
		
		if (stripos($entry, "UNKNOWN") === false) {
		...
		if ($sessionData->geoCoderData[$i]->srcCity[0] === -1 && stripos($entry, "UNKNOWN") === false && !in_array($entry, $geocodingQueue)) {
			array_push($geocodingQueue, $entry);
		}
		...
		if ($sessionData->geoCoderData[$i]->dstCity[0] === -1 && stripos($entry, "UNKNOWN") === false && !in_array($entry, $geocodingQueue)) {
			array_push($geocodingQueue, $entry);
		}
	}
